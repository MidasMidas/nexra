...
class NexraApp:
    def __init__(self):
        ...
        self.db_path = ROOT / self.config["databasePath"]
    ...
    def _load_snapshot(self):
        with self.db_path.open("r", encoding="utf-8-sig") as handle:
        ...
        with self.lock, self.connect() as conn:
            ...
            conn.commit()
        log.info("Loaded Python backend state snapshot from %s", self.db_path)
...
def main():
    ...
    server = ThreadingHTTPServer((host, port), NexraHandler)
    log.info("Nexra Python backend running at http://%s:%s/api", host, port)
